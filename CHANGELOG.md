...
* Added Ed25519 support to `venafi_certificate`: set `privatekey_type: ECDSA` and `privatekey_curve: ed25519` (requires `vcert>=0.22.1`).
...
* Fixed `venafi_policy` crashing with `KeyError: 'policy_spec_path'` on `state: absent` when `policy_spec_path` was omitted (the documented, now-supported usage). The canonical argument is `path` and the `policy_spec_path` alias key is absent when unsupplied, so the module now reads it with a safe default.
* Fixed `venafi_policy` idempotency drift-detection crashing with `TypeError: 'NoneType' object is not iterable` when the platform returns no value for a list field the policy file declares (e.g. `subjectAltNames.uriProtocols`/`ipConstraints`/`domains` against a CIT that does not constrain them). The drift is now reported instead of raising.
* Fixed `venafi_policy` `certificateAuthority` idempotency on CyberArk Certificate Manager, Self-Hosted (TPP). The effective-CA comparison (which defaults an omitted CA to the built-in `DEFAULT_CA`) is a SaaS/NGTS concept; on TPP a folder that locks no CA reads back empty, so the module reported `changed=True` on every run and apply pushed a SaaS CA string into the folder. TPP now diffs `certificateAuthority` only when the file sets a real, non-default CA (SaaS/NGTS behavior unchanged).
* Fixed `venafi_certificate` crashing with a `TypeError` on a local/service CSR when `privatekey_path` was omitted. The generated key is now written to a path derived from `cert_path` (the documented "placed near certificate with key suffix" behavior), instead of attempting to write to `None`.
